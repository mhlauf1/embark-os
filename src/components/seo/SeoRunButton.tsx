...
interface SeoRunButtonProps {
  locationId: string;
  label?: string;
  size?: "sm" | "md";
}

export function SeoRunButton({
  locationId,
  label = "Crawl",
  size = "md",
}: SeoRunButtonProps) {
  ...
  async function handleRun() {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        alert(err?.error ?? `Crawl failed (${res.status})`);
        return;
      }
      router.refresh();
    } catch (err) {
      const message = err instanceof Error ? err.message : "Unknown error";
      alert(`Crawl request failed: ${message}`);
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    ...
    >
      ...
      {loading ? "Crawling..." : label}
    </button>
  );
}

interface SeoRunAllButtonProps {
  locationIds: string[];
}

export function SeoRunAllButton({ locationIds }: SeoRunAllButtonProps) {
  const [loading, setLoading] = useState(false);
    ...
    for (let i = 0; i < total; i++) {
      ...
      try {
        await fetch("/api/seo", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ locationId: locationIds[i] }),
        });
      } catch {
  ...
  return (
    ...
    >
      {loading ? (
        <>
          <Loader2 className="h-3.5 w-3.5 animate-spin" />
          Crawling {progress.current} of {progress.total}...
        </>
      ) : (
        <>
          <Play className="h-3.5 w-3.5" />
